/*
 * 파일 역할 요약:
 * - 친구 도메인 API 라우트를 Drogon 앱에 등록한다.
 * - HTTP 요청을 인증/검증/서비스 호출/JSON 응답으로 연결하는 실제 처리 로직을 구현한다.
 */
...
void FriendController::registerHandlers()
{
    ...
    app.registerHandler(
        ...
        [this](const drogon::HttpRequestPtr &request,
               Callback &&callback,
               const std::string &friendshipIdValue) {
            handleRemoveFriend(request, std::move(callback), friendshipIdValue);
        },
        {drogon::Delete});
}

// FriendMemberDTO를 JSON으로 직렬화한다.
...
// FriendRequestDTO를 JSON으로 직렬화한다.
...
// 세션 토큰을 기반으로 현재 로그인 회원 ID를 조회한다.
...
void FriendController::handleRemoveFriend(const drogon::HttpRequestPtr &request,
                                          Callback &&callback,
                                          const std::string &friendshipIdValue)
{
    const auto memberId = extractCurrentMemberId(request);
    ...
    const auto result = friendService_.removeFriend(*memberId, *friendshipId);
